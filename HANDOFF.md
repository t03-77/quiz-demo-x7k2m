...
2026-09-01 に第1弾を実施（`_overlap_target_{EXAM}.json` / `_予測_overlap2_20260901.md` /
`_bak_overlap2_*_20260901/`）。結果: DOP 0.189→**0.214**、SAP 0.209→**0.246**。
**DEA は担当エージェントがハングして未着手のまま**（対象リストは作成済み。次回再実行）。

第1弾の学び（詳細は `_予測_overlap2_20260901.md`）:
- 指標はトークン Jaccard。**「正解肢の文をコピーして1〜2トークンだけ置換」と明示的に指示**しないと、
  意味的に骨格を共有してもスコアが伸びない（局所 0.19 止まり。丸ごと流用した問題は 0.33〜0.43）
- 誤答を長くすると**「最長は誤答」の癖が出る**（SAP で実際に発生し要確認化 → 最小差13問の
  冗長語削除で公式並みに戻した）。組み直し後は必ず audit_difficulty / audit_strawman を回すこと
- **資格ごとに公式値と比べること**（CLF/AIF は公式自体が低く既に達成済み。全体平均を目標にしない）
