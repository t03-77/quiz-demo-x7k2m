...
### 0b. 問題文の分量（**唯一残っている要確認・2026-08-30 時点**）

`audit_all.py` の要確認はこれ1件だけ。

| 資格 | 公式比 | 状態 |
|---|---|---|
| CLF-C02 | ~~147%~~ → **107%** | **完了（2026-08-30）**。100字以上の63問を1問ずつ読み、正解の決め手になる制約語を残して枕・冗長表現だけを削った。中央値 109→79.5字。パッチは `資料/生成/_patch_clf_len_20260830.json`、予測と実測の照合は `_予測_CLF長さ_20260830.md`、修正前ソースは `_bak_clf_len_20260830/` |
| AIF-C01 | **82%** ★ | 公式より2割短い。残作業 |
| MLA-C01 | **84%** ★ | 同上。残作業 |

AIF-C01 と MLA-C01 は短い側。公式に合わせるならシナリオを足すことになるが、
**水増しにならないよう注意すること**（分量だけ増やしても難易度は上がらない。
これは過去に指摘を受けている）。足すなら「その文脈があると誤答が魅力的になる」
状況説明（鉄則2: 誤答から設計する）に限ること。
