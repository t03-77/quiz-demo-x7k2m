...
### 0b. 問題文の分量（**唯一残っている要確認・2026-08-29 時点**）

`audit_all.py` の要確認はこれ1件だけ。中央値が公式から離れている3資格。

| 資格 | 公式比 | 内容 |
|---|---|---|
| CLF-C02 | **147%** | 公式は中央74字・最短21字。「Amazon S3 の特性はどれですか」のような直接的な知識問題が中心 |
| AIF-C01 | **82%** | 公式より2割短い |
| MLA-C01 | **84%** | 同上 |

CLF-C02 は、外しても問いが成立する31問から「ある企業が〜しています。」の枕を外した
（112字→109字）。**残り71問はシナリオが判断に効いており、機械的には外せない。**
1問ずつ「この文脈は解答に必要か」を判断する作業になる。

AIF-C01 と MLA-C01 は逆に短い。公式に合わせるならシナリオを足すことになるが、
