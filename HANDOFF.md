...
### 1b. 肢どうしの語の重なり（**8資格の組み直し完了・2026-09-03。残りは最終寄せのみ**）

公式の4肢は「同じ骨格・同じ語彙を共有し1〜2点だけ違う集合」。9/1〜9/3 に
**「正解肢の文をコピーして1〜2トークンだけ置換」方式**で8資格・291問656肢の誤答を組み直した。

| 資格 | 公式 | 前 | 後(9/3) | 状態 |
|---|---|---|---|---|
| DVA-C02 | 0.286 | 0.252 | **0.295** | ✅ 到達（最長分布20.6%/72.5%も維持） |
| MLA-C01 | 0.271 | 0.195 | **0.266** | ✅ 到達 |
| SAA-C03 | 0.246 | 0.215 | **0.245** | ✅ 到達 |
| SCS-C03 | 0.285 | 0.193 | **0.243** | 85%充足 |
| SOA-C03 | 0.257 | 0.183 | **0.229** | 89%充足 |
| SAP-C02 | 0.409 | 0.209 | **0.334** | 82%充足 |
| DEA-C01 | 0.349 | 0.214 | **0.246** | 70%充足 |
| DOP-C02 | 0.438 | 0.189 | **0.298** | 68%充足（公式が突出して高い資格） |

残作業: 未達5資格の「次に重なりが薄い30問」への追加パス（確立済み手順の繰り返し。
対象抽出→バックアップ→エージェント→検証→audit）。学び・副作用対処は
`_予測_overlap2_20260901.md` / `_予測_overlap3_20260902.md` 参照。
**副作用の定番はキーワード直結と最長分布**。組み直し後は必ず strawman / difficulty / pattern を回す。
...
### 4. 論点の洗い出し（**全11資格完了・2026-09-03**）

> **分析は終わっている。** 成果物は `資料/生成/_topics_{EXAM}.json`（公式全問の論点一覧・
> 全IDの漏れ重複なしを機械検証済み）と `_topic_gap_{EXAM}.md`（未カバー論点と補強優先順）。

未カバー論点の規模（多い順）: SOA 88 / DEA 54 / SAP 43 / MLA 42 / DOP 35 / DVA 33 /
SCS 32 / CLF 28 / AIF 25 / SAA 26 / AIP 55（8月実施分）。

**次のフェーズはギャップを埋める新問の追加作成**（各 _topic_gap の「補強優先順」から着手。
作問は鉄則1〜8と作問ガイドに従う）。全埋めではなく、公式で複数問出る重要論点を優先する。
ついでに判明した個別課題: DVA orig_068/301 がほぼ同一論点で要統合。自作の3〜4割は
公式サンプルに対応論点がないが大半は試験ガイド範囲内なので削除は不要。
