...
| 種別 | 場所 | サイトに同梱 |
|---|---|---|
| オリジナル問題 1488問（自作） | `data/orig.js` | **する** |
...
| 項目 | 状態 |
|---|---|
| 問題数 | **1488問**（全11資格。各105〜342問。うち56問は2026-09-03 に論点ギャップから新規作成） |
...
### 1b. 肢どうしの語の重なり（**2026-09-03 に第3弾まで完了。残りは DOP と AIF のみ**）

公式の4肢は「同じ骨格・同じ語彙を共有し1〜2点だけ違う集合」。
**「正解肢の文をコピーして1〜2トークンだけ置換」**する方式で、9/1〜9/3 に3巡実施した。

| 資格 | 公式 | 現在 | 状態 |
|---|---|---|---|
| SAP-C02 | 0.409 | **0.397** | ✅ レンジ内 |
| DEA-C01 | 0.349 | **0.332** | ✅ レンジ内 |
| SCS-C03 | 0.285 | **0.276** | ✅ レンジ内 |
| DVA-C02 | 0.286 | **0.295** | ✅ レンジ内 |
| MLA-C01 | 0.271 | **0.266** | ✅ レンジ内 |
| SOA-C03 | 0.257 | **0.245** | ✅ レンジ内 |
| SAA-C03 | 0.246 | **0.243** | ✅ レンジ内 |
| AIP-C01 | 0.247 | **0.273** | ✅ レンジ内 |
| CLF-C02 | 0.119 | **0.142** | ✅ レンジ内（公式が低い資格） |
| **DOP-C02** | 0.438 | **0.386** | ★不足88%。公式が突出して高く、4巡目が要る |
| **AIF-C01** | 0.157 | **0.212** | ★**超過**135%。**下げる**作業が必要 |

**残作業は DOP（上げる）と AIF（下げる）の2資格だけ。**
AIF は公式自体が低い資格なのに自作が高すぎる。誤答の言い回しを散らして下げること。

手順と失敗の記録: `_予測_overlap2_20260901.md` / `_予測_overlap3_20260902.md` / `_予測_overlap4_20260903.md`

**組み直しの副作用は必ず出る。** 過去3回で実際に起きたもの:
- 「最長は誤答」の癖（SAP 9/1）／「最長が正解」の上振れ（DEA 9/3: 27.9%→19.2%に是正）
- キーワード直結の新規発生（9/2 に4問、9/3 に数問）
- 制約の裏返しの新規発生（9/3 SAP-020）
**適用後は必ず audit_all を回す**（語の重なりは 9/3 から audit_all に統合済み）。
