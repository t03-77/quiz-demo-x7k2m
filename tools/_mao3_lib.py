# -*- coding: utf-8 -*-
"""MLA-C01 / AIF-C01 の O-3(正解と酷似した誤答)是正 用の共通ライブラリ。
tools/_mao3_lib.py と tools/_o3b_lib.py を統合したもの（測り方は完全に同一）。
...
EXAMS = {
    "MLA-C01": dict(
        files=["MLA-C01_orig.json", "MLA-C01_orig_b2.json", "MLA-C01_orig_b3.json",
               "MLA-C01_orig_b4.json", "MLA-C01_orig_gap1.json", "MLA-C01_orig_topic1.json"],
        readonly=["mixed_orig_b1.json"]),
    "AIF-C01": dict(
        files=["AIF-C01_orig.json", "AIF-C01_orig_b2.json", "AIF-C01_orig_b3.json",
               "AIF-C01_orig_b4.json", "AIF-C01_orig_gap1.json", "AIF-C01_orig_topic1.json"],
        readonly=[]),
}

EXAM = os.environ.get("O3EXAM", "MLA-C01")
if EXAM not in EXAMS:
    raise SystemExit("O3EXAM は MLA-C01 / AIF-C01 のみ: %r" % EXAM)
...
def set_exam(ex):
    ...
    if ex not in EXAMS:
        raise SystemExit("対象外の資格: %r（MLA-C01/AIF-C01 のみ）" % ex)
