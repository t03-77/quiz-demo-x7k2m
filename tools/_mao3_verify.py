# -*- coding: utf-8 -*-
"""不変項目の照合（正解肢 text を含む）。使い方: python tools/_mao3_verify.py MLA-C01"""
import json, sys, os
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
EX = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("O3EXAM", "MLA-C01")
os.environ["O3EXAM"] = EX
import _mao3_lib as L
L.set_exam(EX)
snap = json.load(open(Path(__file__).resolve().parent / ("_mao3_snapshot_%s.json" % EX), encoding="utf-8"))
