# -*- coding: utf-8 -*-
"""不変項目の照合（正解肢 text を含む）。O3EXAM=AIP-C01 python tools/_sao3_verify.py"""
import json, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
from _sao3_lib import load_mine, exam
ex = exam()
snap = json.load(open(Path(__file__).resolve().parent / ("_sao3_snapshot_%s.json" % ex), encoding="utf-8"))
cur = {q["id"]: q for q in load_mine()}
bad = []
if set(snap) != set(cur):
...
for qid, s in snap.items():
    q = cur.get(qid)
    if not q: continue
    if q["_file"] != s["file"]: bad.append("%s: 所属ファイルが変化" % qid)
    ...
    for a, b in zip(so, qo):
        if a["letter"] != b["letter"]: bad.append("%s: letter が変化 %s->%s" % (qid, a["letter"], b["letter"]))
