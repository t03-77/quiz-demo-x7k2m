# -*- coding: utf-8 -*-
"""O-3是正パッチの適用（MLA-C01 / AIF-C01）。
安全装置(鉄則3): 正解肢への書き込みを検出したら1件も書かずに中断する。
保存形式: 無改変ラウンドトリップがバイト一致する形式を検出し、その形式でのみ書き戻す。
使い方: python tools/_mao3_apply.py MLA-C01 資料/生成/_mao3_patch_xx.json [--dry]
"""
import json, sys, os
...
# 読取専用ファイル(mixed_orig_b1.json 等)の id は index に入れない = パッチできない
RO_IDS = set()
for f in L.READONLY:
    for q in json.load(open(L.GEN / f, encoding="utf-8")):
        if q.get("exam") == EX:
            RO_IDS.add(q["id"])
...
for p_ in patch:
    ...
    seen.add((qid, let))
    if qid in RO_IDS:
        errs.append("%s: **読取専用ファイルの問題**（複数資格が同居）" % qid)
        continue
    if qid not in index:
