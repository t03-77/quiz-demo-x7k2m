...
from _sao3_lib import load_mine, load_off, pairs, overlap, multi_sent, maxsim, sentences, exam
...
O = metrics(load_off())
print()
print("%-16s %8s %8s %8s"%("指標","before","after","公式"))
...
print()
print("[%s] 重なり★不足の下限(公式x0.9) = %.4f / after = %.4f  -> %s"
      % (ex, O["ov"]*0.9, A["ov"], "OK" if A["ov"] >= O["ov"]*0.9 or O["ov"]-A["ov"] < 0.03 else "★NG"))
