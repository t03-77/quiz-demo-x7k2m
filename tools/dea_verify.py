# -*- coding: utf-8 -*-
"""DEA-C01 の書き直しが「触ってはいけない項目」を壊していないか検査する。

dea_snapshot.py が保存した着手前の状態と現在のファイルを突き合わせ、
...
SNAP = BASE / "tools" / "_dea_snapshot.json"
...
def current():
    ...
    for f in sorted(glob.glob(str(BASE / "資料" / "生成" / "*_orig*.json"))):
        for q in json.load(open(f, encoding="utf-8")):
            if q.get("exam") == "DEA-C01" and q.get("set") == "orig":
                cur[q["id"]] = q
    return cur
