# -*- coding: utf-8 -*-
"""SCS-C03 の作り直しが「触ってはいけない項目」を壊していないか検査する。

scs_snapshot.py が保存した着手前の状態と現在のファイルを突き合わせ、
id / exam / set / type / domain / level / n_correct / letter / correct /
正解肢 text の一致を確認する。question は言い換えを認めるため、
一致ではなく変更件数だけを報告する。
...
SNAP = BASE / "tools" / "_scs_snapshot.json"
EXAM = "SCS-C03"
...
def main():
    ...
    bad = []
    if set(snap) != set(cur):
        bad.append(f"問題IDの集合が変化: 消失 {sorted(set(snap)-set(cur))[:5]} / "
                   f"追加 {sorted(set(cur)-set(snap))[:5]}")
    q_changed = 0
    for qid, s in snap.items():
        q = cur.get(qid)
        if not q:
            continue
        for key in ("exam", "set", "type", "domain", "level", "n_correct"):
            if key == "type":
                if q.get("type", "choice") != s["type"]:
                    bad.append(f"{qid}: type が変化")
                continue
            if q.get(key) != s[key]:
                bad.append(f"{qid}: {key} が変化 {s[key]!r} -> {q.get(key)!r}")
        if q.get("question") != s["question"]:
            q_changed += 1
    ...
    print(f"問題文を言い換えた問題: {q_changed}問")
