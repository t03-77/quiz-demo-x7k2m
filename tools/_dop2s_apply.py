# -*- coding: utf-8 -*-
"""DOP-C02 選択肢の2文構造化パッチを適用する。

正解肢の text も書き換える作業なので、dop_apply_patch.py の安全装置は使えない。
代わりに以下を強制する:
  - 正解肢を書き換えるときは "allow_correct": true を明示する
  - text は必ず2文以上（(?<=[。？！]) で分割して2要素以上）
  - id/letter/correct は触らない（照合は _dop2s_verify.py）
  - 改行コードとインデントは既存ファイルに合わせる
使い方: python tools/_dop2s_apply.py 資料/生成/_2s_batchN.json [--dry]
"""
...
def main(patch_path):
    ...
    for f in sorted(glob.glob(str(GEN / "DOP-C02_orig*.json"))):
        ...
        meta[f] = (nl, indent or 2, raw.endswith(b"
"))
        for q in data:
