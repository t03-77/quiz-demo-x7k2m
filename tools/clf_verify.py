# -*- coding: utf-8 -*-
"""CLF-C02 の誤答肢そろえ作業が「触ってはいけない項目」を壊していないか検査する。

clf_snapshot.py が保存した着手前の状態と現在のファイルを突き合わせ、
id / exam / set / type / domain / level / question / n_correct / letter /
correct / 正解肢 text の完全一致を確認する。
今回は問題文を一切触らないため、question も一致必須にしている。
...
def main():
    ...
    for qid, s in snap.items():
        ...
        if not q:
            continue
        for key in ("exam", "set", "type", "domain", "level", "n_correct", "question"):
            if key == "type":
    ...
    print(f"問題文の中央値: {statistics.median(qlens):.0f}字")
