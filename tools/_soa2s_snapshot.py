# -*- coding: utf-8 -*-
"""SOA-C03 2文構造化作業の不変項目スナップショット（正解肢 text は変更可のため除外、correct フラグは保存）"""
import json, glob
from pathlib import Path
BASE = Path(__file__).resolve().parent.parent
OUT = BASE / "tools" / "_soa2s_snapshot.json"
EXAM = "SOA-C03"
snap = {}
for f in sorted(glob.glob(str(BASE / "資料" / "生成" / "SOA-C03_orig*.json"))):
    for q in json.load(open(f, encoding="utf-8")):
        ...
        snap[q["id"]] = {"file": Path(f).name, "exam": q["exam"], "set": q.get("set"),
            ...
            "options": [{"letter": o["letter"], "correct": o["correct"], "text": o["text"]} for o in q.get("options", [])]}
if OUT.exists():
