# -*- coding: utf-8 -*-
"""O-3是正作業の不変項目スナップショット(SAP-C02 / AIP-C01)。
鉄則3: id/exam/set/type/domain/level/question/n_correct/letter/correct は不変。
さらに本作業では「正解肢の text も触らない」ため、正解肢 text も保存して照合する。
使い方: O3EXAM=AIP-C01 python tools/_sao3_snapshot.py"""
import json, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
from _sao3_lib import load_mine, exam
ex = exam()
OUT = Path(__file__).resolve().parent / ("_sao3_snapshot_%s.json" % ex)
snap = {}
for q in load_mine():
