# -*- coding: utf-8 -*-
"""SAA-C03 の「正解が最長」の割合を公式模試の水準に近づける。
...
def load():
    ...
    for f in sorted(glob.glob(str(GEN / "*_orig*.json"))):
        ...
        for q in data:
            if q.get("exam") == "SAA-C03" and q.get("type", "choice") == "choice":
                index[q["id"]] = (f, q)
...
def main():
    ...
    for qid in ids:
        ...
        if max(len(o["text"]) for o in q["options"]) == max(len(o["text"]) for o in cor):
            already += 1
    need = max(0, round(len(ids) * TARGET_PCT / 100) - already)
    print(f"すでに正解が最長: {already}/{len(ids)}問 -> あと{need}問戻す")
    done = 0
    touched = set()
    for qid in ids:
        if done >= need:
