# -*- coding: utf-8 -*-
"""不変項目の照合(正解肢 text を含む)。使い方: python tools/_o3b_verify.py CLF-C02"""
import json, sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parent))
from _o3b_lib import load_mine
exam = sys.argv[1]
snap = json.load(open(Path(__file__).resolve().parent / ("_o3b_snapshot_%s.json" % exam), encoding="utf-8"))
cur = {q["id"]: q for q in load_mine(exam)}
bad = []
if set(snap) != set(cur):
...
print("%s 照合 %d問 / 不一致 %d件" % (exam, len(cur), len(bad)))
for b in bad[:30]:
    print("  " + b)
