# -*- coding: utf-8 -*-
"""SOA-C03 選択肢の2文構造化パッチを適用する。

正解肢の text も書き換える作業なので、soa_apply_patch.py 系の安全装置は使えない。
代わりに以下を強制する:
  - 正解肢を書き換えるときは "allow_correct": true を明示する
  - text は必ず2文以上（(?<=[。？！]) で分割して2要素以上）
  - id/letter/correct/question 等は触らない（照合は _soa2s_verify.py）
  - 改行コード・インデント・末尾改行は既存ファイルに合わせる（無改変ラウンドトリップがバイト一致）
使い方: python tools/_soa2s_apply.py 資料/生成/_2s_soa_NN.json [--dry]
"""
...
def cls(q):
    op = q["options"]
    cor = [len(o["text"]) for o in op if o["correct"]]; wr = [len(o["text"]) for o in op if not o["correct"]]
    if not cor or not wr: return "?"
    m = max(cor+wr); a = max(cor)==m; b = max(wr)==m
    return "hi" if (a and not b) else ("lo" if (b and not a) else "same")

def fmt(path):
    raw = Path(path).read_bytes(); txt = raw.decode("utf-8")
    nl = "\r\n" if b"\r\n" in raw else "\n"
    lines = txt.replace("\r\n","\n").split("\n")
...
def main(patch_path):
    ...
    files, meta, index = {}, {}, {}
    for f in sorted(glob.glob(str(GEN / "SOA-C03_orig*.json"))):
        raw = Path(f).read_bytes()
        data = json.loads(raw.decode("utf-8"))
        nl, ind, tail = fmt(f)
        # 無改変ラウンドトリップのバイト一致を毎回確認する
        chk = (json.dumps(data, ensure_ascii=False, indent=ind) + tail).replace("\n", nl).encode("utf-8")
        if chk != raw:
            print("NG: %s は無改変ラウンドトリップがバイト一致しません" % f); return 1
        files[f] = data; meta[f] = (nl, ind, tail)
        for q in data:
            if q.get("exam") == EXAM: index[q["id"]] = (f, q)
    before = {qid: (ov(q), cls(q), [len(o["text"]) for o in q["options"]]) for qid, (_, q) in index.items()}
    ...
    for p in patches:
        ...
        if "text" in p:
            ...
            r = len(t) / max(1, len(o["text"]))
            if not (0.80 <= r <= 1.30):
                errors.append("%s[%s]: 長さが %d→%d (%.0f%%)。0.80〜1.30 に収める" % (qid, p["letter"], len(o["text"]), len(t), r*100)); continue
            if t != o["text"]:
    ...
    for qid in sorted({p["id"] for p in patches}):
        ...
        c2 = cls(q)
        if c2 != bc: flip += 1
        print("  %s 2文%d/%d 長さ%s→%s 最長%s→%s 重なり%.3f→%.3f" % (qid, n2, len(q["options"]),
              ",".join(map(str, bl)), ",".join(str(len(o["text"])) for o in q["options"]), bc, c2, bo, ov(q)))
    print("最長クラスが変化した問題: %d" % flip)
    if "--dry" in sys.argv:
        print("(--dry) 書き込みなし"); return 0
    for f in touched:
        nl, ind, tail = meta[f]
        s = json.dumps(files[f], ensure_ascii=False, indent=ind) + tail
